...
-- source
defineProperty("bat_on_bus", globalPropertyi("tu154b2/custom/switchers/eng/bat1_on")) -- батарея подключена в сеть
defineProperty("bat_source", globalPropertyi("tu154b2/custom/elec/bat_is_source_1")) -- батарея является источником
defineProperty("bat_amp_bus", globalPropertyf("tu154b2/custom/elec/bat_amp_1")) -- нагрузка батареи
defineProperty("bat_amp_cc", globalPropertyf("tu154b2/custom/elec/bat_cc_1")) -- ток заряда батареи
defineProperty("bat_volt_bus", globalPropertyf("tu154b2/custom/elec/bat_volt_1")) -- напряжение батареи
defineProperty("bat_thermo", globalPropertyf("tu154b2/custom/elec/bat_therm_1")) -- напряжение батареи

defineProperty("bat_fail", globalPropertyi("tu154b2/custom/failures/bat_1_fail")) -- отказ батареи
defineProperty("bat_kz", globalPropertyi("tu154b2/custom/failures/bat_1_kz")) -- тепловой разгон

defineProperty("bus_volt", globalPropertyf("tu154b2/custom/elec/bus27_volt_left")) -- напряжение в сети.

defineProperty("cockpit_temp", globalPropertyf("tu154b2/custom/thermo/cockpit_temp")) -- температура в кабине
defineProperty("bat_tot", globalPropertyf("tu154b2/custom/elec/bat_amp_tot_1"))

-- other datarefs
defineProperty("frame_time", globalPropertyf("tu154b2/custom/time/frame_time")) -- flight time

defineProperty("sim_bat_on", globalProperty("sim/cockpit2/electrical/battery_on[0]")) -- sim battery on
defineProperty("apu_start_seq", globalPropertyi("tu154b2/custom/elec/apu_start_seq"))
	...
	if passed > 0 then
		...
		if bat_on == 1 then   -- all of calculations can be skipped if batteries are off
			...
			if get(bat_source) == 1 then
				...
				if get(bus_volt) > bat_volt and get(apu_start_seq)==0 then
				...
				local chrg=((MAX_BAT_CAPACITY - bat_capacity) * BAT_CURRENT_COEF + interpolate(current_table, kz_timer))*3
				set(bat_amp_cc, chrg)   -- bat current depends on their charge
				tot_amp=-chrg
			end
