...
defineProperty("emer_brake", globalPropertyf("tu154b2/custom/controlls/brake_emerg")) -- emer brk




defineProperty("turn_rate_ind", globalPropertyf("tu154b2/custom/gauges/misc/turn_rate_ind")) -- индикатор разворота
defineProperty("big_course_needle", globalPropertyf("tu154b2/custom/gauges/compas/big_course_needle")) -- стрелка "самолетик"

-- flaps and stab


defineProperty("stab_man_cap", globalPropertyi("tu154b2/custom/controll/stab_man_cap")) -- положение крышки управления стабилизатором
...
defineProperty("int_pitch_trim", globalPropertyf("tu154b2/custom/trimmers/int_pitch_trim"))
...
-- altitude
defineProperty("rv5_alt", globalPropertyf("tu154b2/custom/misc/rv5_alt_left"))
defineProperty("pressure_L", globalPropertyf("tu154b2/custom/gauges/alt/vbe_press_left"))
defineProperty("pressure_R", globalPropertyf("tu154b2/custom/gauges/alt/vbe_press_right"))
...
defineProperty("rv_test_btn", globalPropertyf("tu154b2/custom/gauges/alt/radioalt_button_left"))  -- Test button
...
function regular_talk()
	...
	if not v_var["flaps_retr_said"] and v_var["flaps_diff"] < -0.0001 and math.abs(v_var["flaps_last_L"] - v_var["flaps_last_R"]) < 1 and flight_status ~=8 then
	...
	-- slats retracting
	if not v_var["slats_retr_said"] and v_var["slats_diff"] < -0.0001 and (v_var["flaps_retr_said"] or get(slat_man) == -1) and flight_status ~=8 then
	...
	if not v_var["stab_move_said"] and math.abs(v_var["stab_diff"]) > 0.001 and v_var["trim_count"] > 2 and (v_var["flaps_ext_said"] or v_var["flaps_retr_said"]) and v_var["stab_count"] > 1 and flight_status ~=8 then
...
elseif flight_status == 1 then	
    ...
    if not v_var["brk_chk_ok_said"] then
        brake_chk_summ = brake_chk_summ +(get(l_brake_add)+get(r_brake_add)+get(emer_brake))*passed_time
    else
        brake_chk_summ = 0
    end
	...
	-- checking brakes
	if brake_chk_summ > 1.9 and not v_var["brk_chk_ok_said"] and v_var["check_brk_said"] then
