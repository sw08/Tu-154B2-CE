...
function update()
	...
	if get(ismaster)~=1 then
		...
		local nws_power=bool2int((get(bus27_volt_left) > 13 or get(bus27_volt_right) > 13) and nws_on == 1 and grnd)
		local nose_load=math.max(5*math.pow(compression,2)-4*compression+1,0.2)
		local turn_max = (math.min(2+get(groundspeed),2.5)/2.5*5*nose_load*press/210) -- turn slower when standing still or nose heavy or low pressure
		...
		local pedal_steer=get(have_pedals)==1
		local tiller=get(tiller_val)*63
		local pedals=get(joy_yaw)*10
		if turn_mode == 1 and pedal_steer then -- use tiller			
		...
		gear_turn_pos = gear_turn_pos + turn_rate*nws_power*passed*10 -gear_turn_pos*passed*math.max((0.5-compression*3),0)
		if not pbConnect or nws_power>0 then
			set(tire_steer_command_deg,gear_turn_pos)
		end
		if nws_power>0 then
			set(tiller_angle,gear_turn_pos/63*turn_mode)
		else
			set(tiller_angle,tiller/63)
		end
	end
