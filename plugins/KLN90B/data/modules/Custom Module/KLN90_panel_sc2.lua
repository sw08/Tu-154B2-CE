...
defineProperty("n_sats", globalPropertyi("tu154b2/custom/kln90/kln_n_sats"))
...
local cagevisible = 0
local sved=0
local B_L_Angle_3D_prev=0
local L_Angle_3D_prev=0
local B_R_Angle_3D_prev=0
local R_Angle_3D_prev=0
...
local power_prev=0
function update()
	if get(ismaster)~=0 then
		if get(B_L_Angle_3D)~=B_L_Angle_3D_prev then
			controls["lknobl"] = get(B_L_Angle_3D)-B_L_Angle_3D_prev
			controls["blknobsangle"] = controls["blknobsangle"] + get(B_L_Angle_3D)-B_L_Angle_3D_prev
		end
		if get(L_Angle_3D)~=L_Angle_3D_prev then
			controls["lknobs"] = get(L_Angle_3D)-L_Angle_3D_prev 
			controls["lknobsangle"] = controls["lknobsangle"] + get(L_Angle_3D)-L_Angle_3D_prev 
		end
		
		if get(B_R_Angle_3D)~=B_R_Angle_3D_prev then
			controls["rknobl"] = get(B_R_Angle_3D)-B_R_Angle_3D_prev 
			controls["brknobsangle"] = controls["brknobsangle"] + get(B_R_Angle_3D)-B_R_Angle_3D_prev 
		end
		if get(R_Angle_3D)~=R_Angle_3D_prev then
			controls["rknobs"] = get(R_Angle_3D)-R_Angle_3D_prev
			controls["rknobsangle"] = controls["rknobsangle"] + get(R_Angle_3D)-R_Angle_3D_prev
		end
		
		
		B_L_Angle_3D_prev=get(B_L_Angle_3D)
		L_Angle_3D_prev=get(L_Angle_3D)
		B_R_Angle_3D_prev=get(B_R_Angle_3D)
		R_Angle_3D_prev=get(R_Angle_3D)
	end
	...
	if 0 == phase then
		if get(ismaster)~=1 then
			if get(ismaster)==0 then
				...
				local ang=get(B_L_Angle_3D)-1
				set(B_L_Angle_3D , ang)
			end
	...
	if 0 == phase then
		if get(ismaster)~=1 then
			if get(ismaster)==0 then
				...
				local ang=get(B_R_Angle_3D)-1
				set(B_R_Angle_3D , ang)
			end			
		end
